...
export class CriarReservaComponent implements OnInit, OnDestroy {
  ...
  onAddReserva() {
    ...
    if (this.criarReserva.value.dataFim < this.criarReserva.value.dataInicio) {
      ...
      console.log(
        'A data de início deve ser anterior à data de fim da reserva'
      );
      return;
      ...
      if (this.criarReserva.value.tipoEspaco === 'Openspace') {
        ...
        if (this.numSecretOpenSpace - this.numTotalSecret > 0) {
          this.reservaService.addReserva(
            ...
            (this.custo =
              (this.calculoCustoPessoas(this.criarReserva.value.numComp) +
                this.calculoCustoExtras(
                  this.criarReserva.value.tele,
                  this.criarReserva.value.correio,
                  this.criarReserva.value.internet
                )) *
              this.calculoCustoDatas(
                this.criarReserva.value.dataInicio,
                this.criarReserva.value.dataFim
              ))
          );
        ...
        if (this.numSalasReuniao - this.numTotalReuniao > 0) {
          this.reservaService.addReserva(
            ...
            (this.estado = Estado.PENDENTE),
            (this.custo =
              this.calculoCustoPessoas(this.numCompReuniao) *
              this.calculoCustoDatas(
                this.criarReserva.value.dataInicio,
                this.criarReserva.value.dataFim
              ))
          ...
          console.log(this.numSalasReuniao);
          console.log(this.numTotalReuniao);
        ...
        if (this.numSalasFormacao - this.numTotalFormacao > 0) {
          this.reservaService.addReserva(
            ...
            (this.estado = Estado.PENDENTE),
            (this.custo =
              this.calculoCustoPessoas(this.numCompFormacao) *
              this.calculoCustoDatas(
                this.criarReserva.value.dataInicio,
                this.criarReserva.value.dataFim
              ))
  ...
  taxasELimitesEspaco() {
    ...
    this.espacoService.getEspacoLimit().subscribe(data => {
      this.taxaSecretaria = data['espacos'].map(x => x.taxaSecretaria ).reduce((x, y) => x + y);
      this.taxaReuniao = data['espacos'].map(x => x.taxaReuniao).reduce((x, y) => x + y);
      this.taxaFormacao = data['espacos'].map(x => x.taxaFormacao).reduce((x, y) => x + y);
      this.taxaTele = data['espacos'].map(x => x.taxaTele).reduce((x, y) => x + y);
      this.taxaCorreio = data['espacos'].map(x => x.taxaCorreio).reduce((x, y) => x + y);
      this.taxaInternet = data['espacos'].map(x => x.taxaInternet).reduce((x, y) => x + y);
      this.numSalasReuniao = data['espacos'].map(x => x.numSalaReuniao).reduce((x, y) => x + y);
      this.numSalasFormacao = data['espacos'].map(x => x.numSalaFormacao).reduce((x, y) => x + y);
      this.numSecretOpenSpace = data['espacos'].map(x => x.numSecretOpenSpace).reduce((x, y) => x + y);
    });
  }
  ...
  secretariasUsadas() {
    this.espacoService.getSecretarias().subscribe(data => {
      this.numTotalSecret = data['secrets']
        .map(x => x.numComp)
        .reduce((x, y) => x + y);
    });
  ...
  salaReuniao() {
    this.espacoService.getSalaReuniao().subscribe(data => {
      this.numTotalReuniao = data['salasR']
        .map(x => x.numComp)
        .reduce((x, y) => x + y);
    });
  ...
  salaFormacao() {
    this.espacoService.getSalaFormacao().subscribe(data => {
      this.numTotalFormacao = data['salasF']
        .map(x => x.numComp)
        .reduce((x, y) => x + y);
    });
